...
export default function Admin() {
  ...
  const [modalEditarBarbero,setModalEditarBarbero] = useState(null);
  ...
  // ── CITAS ──
  const handleCancelarCita = (id) => setModalConfirmar({
    mensaje: '¿Cancelar esta cita?',
    onConfirmar: async () => {
      try { await api.post(`/citas/${id}/cancelar/`); cargarDatos(); mostrarToast('Cita cancelada.', 'exito'); }
      catch { mostrarToast('Error al cancelar la cita.', 'error'); }
    },
  });

  const handleCompletarCita = (id) => setModalConfirmar({
    mensaje: '¿Marcar esta cita como completada?',
    onConfirmar: async () => {
      try { await api.patch(`/citas/${id}/completar/`); cargarDatos(); mostrarToast('Cita completada.', 'exito'); }
      catch { mostrarToast('Error al completar la cita.', 'error'); }
    },
  });
  ...
  const abrirEditarBarbero = (barbero) => {
    setFormEditar({ nombre: barbero.nombre, especialidad: barbero.especialidad, descripcion: barbero.descripcion || '', foto: null });
  ...
  const handleEditarBarbero = async (e) => {
    ...
    try {
      await api.patch(`/barberos/${modalEditarBarbero.id}/`, data, { headers: { 'Content-Type': 'multipart/form-data' } });
      setModalEditarBarbero(null);
      cargarDatos();
      mostrarToast('Barbero actualizado correctamente.', 'exito');
    } catch { mostrarToast('Error al actualizar el barbero.', 'error'); }
  };

  const handleEliminarBarbero = (id) => setModalConfirmar({
    mensaje: '¿Eliminar este barbero? Esta acción no se puede deshacer.',
    onConfirmar: async () => {
      try { await api.delete(`/barberos/${id}/`); cargarDatos(); mostrarToast('Barbero eliminado.', 'exito'); }
      catch { mostrarToast('Error al eliminar el barbero.', 'error'); }
    },
  });

  // ── USUARIOS ──
  const handleEliminarUsuario = (id, username) => setModalConfirmar({
    mensaje: `¿Eliminar al usuario "${username}"? Esta acción no se puede deshacer.`,
    onConfirmar: async () => {
      try { await api.delete(`/usuarios/${id}/`); cargarDatos(); mostrarToast('Usuario eliminado.', 'exito'); }
  ...
  const rolColor = (r) => ({
    admin:   { bg: 'rgba(100,150,255,0.2)', color: '#aac4ff', border: 'rgba(100,150,255,0.4)' },
    barbero: { bg: 'rgba(100,255,180,0.2)', color: '#6fffc0', border: 'rgba(100,255,180,0.4)' },
    cliente: { bg: 'rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.7)', border: 'rgba(255,255,255,0.2)' },
  }[r] || {});
  ...
  const accionColor = (a) => ({
    ...
    cancelar: '#ffaaaa',
    completar:'#6fffc0',
    login:    'rgba(255,255,255,0.5)',
    logout:   'rgba(255,255,255,0.4)',
  }[a] || '#fff');

  const nombreUsuario = (id) => usuarios.find(u => u.id === id)?.username || `#${id}`;
          ...
          {sidebarItems.map((item) => (
            <button key={item.key}
              style={{
                ...
                color:      seccion === item.key ? '#fff' : 'rgba(255,255,255,0.7)',
            ...
            <>
                ...
                <div>
                  ...
                  <div style={s.tabla}>
                    <div style={s.tablaHeader}>
                      <span>Cliente</span><span>Barbero</span><span>Fecha</span><span>Hora</span><span>Estado</span><span>Acciones</span>
                    </div>
                    {citas.map((cita) => {
                      ...
                      return (
                        <div key={cita.id} style={s.tablaFila}>
                          ...
                          <span><span style={{ ...s.badge, background: col.bg, color: col.color, border: `1px solid ${col.border}` }}>{cita.estado}</span></span>
                          <span style={s.accionesCell}>
                            {cita.estado === 'pendiente' && (
                              <>
                                <button style={s.btnCompletar} onClick={() => handleCompletarCita(cita.id)}>✓</button>
                                <button style={s.btnCancelar}  onClick={() => handleCancelarCita(cita.id)}>✕</button>
                              </>
                            )}
                          </span>
                        </div>
              ...
              {seccion === 'barberos' && (
                <div>
                  <div style={s.seccionHeader}>
                    ...
                    <button style={s.btnAgregar} onClick={() => setModalBarbero(true)}>+ Nuevo barbero</button>
                  </div>
                  <div style={s.grid}>
                    {barberos.map((barbero) => (
                      <div key={barbero.id} style={s.barberoCard}>
                        ...
                        <div style={s.barberoInfo}>
                          ...
                          {barbero.descripcion && <p style={s.barberoDesc}>{barbero.descripcion}</p>}
                        </div>
                        <div style={s.barberoAcciones}>
                          <button style={s.btnEditar}   onClick={() => abrirEditarBarbero(barbero)}>Editar</button>
                          <button style={s.btnEliminar} onClick={() => handleEliminarBarbero(barbero.id)}>Eliminar</button>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <div style={s.tabla}>
                    <div style={{ ...s.tablaHeader, gridTemplateColumns: 'repeat(5, 1fr)' }}>
                      <span>ID</span><span>Usuario</span><span>Email</span><span>Rol</span><span>Acción</span>
                    </div>
                    {usuarios.map((usuario) => {
                      ...
                      return (
                        <div key={usuario.id} style={{ ...s.tablaFila, gridTemplateColumns: 'repeat(5, 1fr)' }}>
                          ...
                          <span><span style={{ ...s.badge, background: rc.bg, color: rc.color, border: `1px solid ${rc.border}` }}>{usuario.rol}</span></span>
                          <span>
                            {usuario.rol !== 'admin' && (
              ...
              {seccion === 'auditoria' && (
                <div>
                  ...
                  <p style={s.subtitulo}>Últimas 200 acciones registradas en el sistema.</p>
                  <div style={s.tabla}>
                    <div style={{ ...s.tablaHeader, gridTemplateColumns: '1fr 1fr 1fr 2fr 1.5fr' }}>
                      <span>Usuario</span><span>Acción</span><span>Entidad</span><span>Detalle</span><span>Fecha</span>
                    </div>
                    {auditoria.map((log) => (
                      <div key={log.id} style={{ ...s.tablaFila, gridTemplateColumns: '1fr 1fr 1fr 2fr 1.5fr' }}>
                        <span style={s.tablaCell}>{log.usuario_nombre || '—'}</span>
                        <span style={{ ...s.tablaCell, color: accionColor(log.accion), fontWeight: '700' }}>
                          {log.accion}
                        </span>
                        <span style={s.tablaCell}>{log.entidad} {log.entidad_id ? `#${log.entidad_id}` : ''}</span>
                        <span style={{ ...s.tablaCell, color: 'rgba(255,255,255,0.6)', fontSize: '12px' }}>{log.detalle}</span>
                        <span style={{ ...s.tablaCell, fontSize: '12px', color: 'rgba(255,255,255,0.5)' }}>
            ...
            <form onSubmit={handleEditarBarbero} style={s.form}>
              ...
              <div style={s.inputGroup}>
                ...
                <textarea style={{ ...s.input, resize: 'vertical', minHeight: '80px' }} value={formEditar.descripcion}
                  onChange={(e) => setFormEditar({ ...formEditar, descripcion: e.target.value })} placeholder="Descripción del barbero..." />
              </div>
              <div style={s.inputGroup}>
        ...
        <div style={s.modalOverlay}>
          <div style={{ ...s.modal, maxWidth: '380px', textAlign: 'center' }}>
            ...
            <div style={{ ...s.formBotones, justifyContent: 'center' }}>
              ...
              <button style={s.btnConfirmar} onClick={() => { modalConfirmar.onConfirmar(); setModalConfirmar(null); }}>Confirmar</button>
            </div>
          </div>
        </div>
      )}
...
const s = {
  ...
  titulo:   { fontSize: '26px', fontWeight: '800', color: '#fff', margin: '0 0 8px' },
  subtitulo:{ fontSize: '14px', color: 'rgba(255,255,255,0.6)', margin: '0 0 24px' },
  ...
  btnCompletar: {
    padding: '6px 10px', background: 'rgba(100,255,180,0.15)',
    border: '1px solid rgba(100,255,180,0.35)', borderRadius: '8px',
    color: '#6fffc0', fontSize: '13px', cursor: 'pointer', fontFamily: "'Nunito', sans-serif", fontWeight: '700',
  },
  btnCancelar: {
    padding: '6px 10px', background: 'rgba(255,100,100,0.15)',
    border: '1px solid rgba(255,100,100,0.35)', borderRadius: '8px',
    color: '#ffaaaa', fontSize: '12px', cursor: 'pointer', fontFamily: "'Nunito', sans-serif", fontWeight: '600',
  },
  ...
  barberoAvatar: {
    ...
    fontSize: '18px', fontWeight: '800', color: '#fff', flexShrink: 0, overflow: 'hidden',
  },
  ...
  btnEditar: {
    padding: '8px 16px', background: 'rgba(100,150,255,0.15)',
    border: '1px solid rgba(100,150,255,0.35)', borderRadius: '10px',
    color: '#aac4ff', fontSize: '13px', cursor: 'pointer', fontFamily: "'Nunito', sans-serif", fontWeight: '600',
  },
  btnEliminar: {
    padding: '8px 16px', background: 'rgba(255,100,100,0.15)',
    border: '1px solid rgba(255,100,100,0.35)', borderRadius: '10px',
    color: '#ffaaaa', fontSize: '13px', cursor: 'pointer', fontFamily: "'Nunito', sans-serif", fontWeight: '600',
  },
  modalOverlay: {
