...
const DURACIONES = Array.from({ length: 10 }, (_, i) => {
  const min = 30 + i * 10;
  return { valor: String(min), label: min < 60 ? `${min} min` : min === 60 ? '1 hora' : `${Math.floor(min/60)}h ${min%60 > 0 ? min%60+'min' : ''}`.trim() };
});
...
export default function Barbero() {
  ...
  const [formTurnos, setFormTurnos] = useState({
    fecha: '', hora_inicio: '', hora_fin: '', duracion: '30',
  });
  ...
  const cargarDatos = async () => {
    try {
      ...
      const miBarbero = barberosRes.data.find(b => b.usuario === perfilRes.data.id);
      if (miBarbero) {
        setBarberoId(miBarbero.id);
        const horariosRes = await api.get(`/horarios/?barbero=${miBarbero.id}`);
  ...
  const actualizarPreview = (form) => {
    if (form.hora_inicio && form.hora_fin && form.duracion) {
      setPreview(generarTurnos(form.hora_inicio, form.hora_fin, parseInt(form.duracion)));
    } else {
      setPreview([]);
    }
  };
  ...
  const handleGenerarTurnos = async (e) => {
    e.preventDefault();
    if (!barberoId) { mostrarToast('Tu usuario no está vinculado a un barbero.', 'error'); return; }
    if (preview.length === 0) { mostrarToast('No se generaron turnos. Verifica los horarios.', 'error'); return; }
    try {
      await Promise.all(preview.map(t => api.post('/horarios/', {
        barbero: barberoId, fecha: formTurnos.fecha,
        hora_inicio: t.hora_inicio, hora_fin: t.hora_fin, disponible: true,
      })));
            ...
            <form onSubmit={handleGenerarTurnos} style={s.form}>
              ...
              <div style={s.formRow}>
                <TimePicker
                  ...
                  value={formTurnos.hora_inicio}
                  onChange={(v) => handleFormChange('hora_inicio', v)}
                  required
                />
                <TimePicker
                  label="Hora de salida"
                  value={formTurnos.hora_fin}
                  onChange={(v) => handleFormChange('hora_fin', v)}
                  required
                />
              </div>

              {/* Selector de duración siempre visible */}
              <div style={s.inputGroup}>
                <label style={s.label}>Duración por corte</label>
                <div style={s.duracionGrid}>
                  {DURACIONES.map(({ valor, label }) => (
                    <button
                      key={valor}
                      type="button"
                      style={{
                        ...s.duracionBtn,
                        background: formTurnos.duracion === valor ? 'rgba(255,255,255,0.9)' : 'rgba(255,255,255,0.1)',
                        color:      formTurnos.duracion === valor ? '#0a4f7a'               : '#fff',
                        border:     formTurnos.duracion === valor ? '1px solid rgba(255,255,255,0.9)' : '1px solid rgba(255,255,255,0.2)',
                        fontWeight: formTurnos.duracion === valor ? '800' : '500',
                      }}
                      onClick={() => handleFormChange('duracion', valor)}
                    >
                      {label}
                    </button>
                  ))}
              ...
              {preview.length > 0 && (
                <div style={s.preview}>
                  ...
                  <div style={s.previewGrid}>
                    {preview.map((t, i) => (
                      <span key={i} style={s.previewTurno}>{t.hora_inicio} — {t.hora_fin}</span>
                    ))}
                  </div>
                </div>
              )}
              ...
              <div style={s.formBotones}>
                <button type="button" style={s.btnCancelarModal}
                  onClick={() => { setMostrarGenerador(false); setPreview([]); }}>
                  Cancelar
                </button>
                <button type="submit"
                  style={{ ...s.btnConfirmar, opacity: preview.length > 0 ? 1 : 0.5 }}
...
const s = {
  ...
  titulo:  { fontSize: '26px', fontWeight: '800', color: '#fff', margin: '0 0 24px' },
  ...
  aviso: {
    ...
    borderRadius: '12px', padding: '14px 18px', color: '#ffe066', fontSize: '14px', marginBottom: '24px',
  },
  ...
  turnoCard: {
    background: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(16px)',
    border: '1px solid rgba(255,255,255,0.18)', borderRadius: '14px', padding: '14px 16px',
  },
  ...
  modal: {
    ...
    maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 12px 48px rgba(0,0,0,0.35)',
  },
  ...
  duracionGrid: { display: 'flex', flexWrap: 'wrap', gap: '8px' },
  duracionBtn:  {
  ...
  previewTurno:  {
    ...
    borderRadius: '20px', padding: '4px 12px', fontSize: '12px', color: '#fff', fontWeight: '600',
  },
