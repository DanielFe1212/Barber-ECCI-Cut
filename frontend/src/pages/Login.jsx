  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!err.response) {
        ...
        else                        setError('Datos inválidos. Verifica los campos.');
      } else if (status === 403) {
        ...
        <div style={s.glass}>
          ...
          <form onSubmit={handleSubmit} style={s.form}>
            <div style={s.inputGroup}>
              ...
              <input
                style={s.input} type="text" name="username"
                placeholder="Tu usuario" value={form.username}
...
const s = {
  ...
  brand: { textAlign: 'center', color: '#fff', position: 'relative', zIndex: 1 },
  logoCircle: {
    width: '90px', height: '90px',
    background: 'rgba(255,255,255,0.2)', border: '2px solid rgba(255,255,255,0.4)',
    borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center',
    fontSize: '40px', margin: '0 auto 20px', backdropFilter: 'blur(10px)',
  },
  brandName: {
    fontSize: '38px', fontWeight: '800', color: '#fff',
    margin: '0 0 8px', textShadow: '0 2px 12px rgba(0,0,0,0.2)', letterSpacing: '0.5px',
  },
  brandSlogan: { fontSize: '16px', color: 'rgba(255,255,255,0.75)', fontStyle: 'italic', margin: 0 },
  ...
  subtitulo: { fontSize: '15px', color: 'rgba(255,255,255,0.7)', margin: '0 0 32px' },
  errorBox: {
    background: 'rgba(255,80,80,0.2)', border: '1px solid rgba(255,100,100,0.4)',
    borderRadius: '10px', padding: '12px 16px', marginBottom: '20px',
    color: '#ffcccc', fontSize: '14px',
  },
  form: { display: 'flex', flexDirection: 'column', gap: '18px' },
  inputGroup: { display: 'flex', flexDirection: 'column', gap: '6px' },
  label: { fontSize: '13px', fontWeight: '700', color: 'rgba(255,255,255,0.85)', letterSpacing: '0.3px' },
  input: {
  ...
  boton: {
    padding: '14px', background: 'rgba(255,255,255,0.9)', color: '#0a4f7a',
    border: 'none', borderRadius: '12px', fontSize: '16px', fontWeight: '800',
    cursor: 'pointer', marginTop: '6px', fontFamily: "'Nunito', sans-serif", transition: 'all 0.2s',
  },
  divider: { display: 'flex', alignItems: 'center', gap: '12px', margin: '24px 0 18px' },
  dividerLine: { flex: 1, height: '1px', background: 'rgba(255,255,255,0.2)', display: 'block' },
  dividerText: { color: 'rgba(255,255,255,0.5)', fontSize: '13px' },
  footerText: { textAlign: 'center', fontSize: '14px', color: 'rgba(255,255,255,0.7)', margin: 0 },
